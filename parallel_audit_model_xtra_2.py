...
def compute_per_sample_gradient_analysis(model, X, y, device):
    ...
    # We'll use a manual loop for safety and simplicity since N is small for analysis
    grads = []
    ...
    for i in range(len(X)):
        ...
        # For efficiency, we can just grab them and flatten
        loss.backward()
...
def train_model(model_name, X, y, X_target, y_target, epsilon, delta, max_grad_norm, 
    ...
    for epoch in range(n_epochs):
        ...
        # --- Gradient Space Dynamics Analysis ---
        if rank == 0 and epoch % 10 == 0:
            print(f"\n--- Epoch {epoch} Gradient Analysis ---")
            # 1. Sample ~100 random training points
            analysis_indices = np.random.choice(len(dataset), min(100, len(dataset)), replace=False)
            ...
            # 2. Compute gradients
            g_train = compute_per_sample_gradient_analysis(model, X_analysis, y_analysis, device)
            
            if len(g_train) > 0:
                # 3. Compute canary gradient
                # Use target_X/target_y passed to train_model
                g_canary = compute_per_sample_gradient_analysis(model, X_target.to(device), y_target.to(device), device)
                ...
                canary_grad_norm = np.linalg.norm(g_canary, axis=1)[0]
                
                # Normalize projections by full norm roughly
                train_alignment = np.linalg.norm(train_grad_proj, axis=1) / (train_grad_norm + 1e-10)
                canary_alignment = np.linalg.norm(canary_grad_proj, axis=1)[0] / (canary_grad_norm + 1e-10)
                ...
                print(f"  Canary Gradient Alignment:       {canary_alignment:.4f}")
                
                is_low_alignment = canary_alignment < np.mean(train_alignment) - np.std(train_alignment)
...
def main():
    ...
    if rank == 0:
        ...
        canary_error = np.mean((canary_flat - canary_recon)**2, axis=1)[0]
        ...
        print(f"Canary Reconstruction Error:    {canary_error:.6f}")
        
        is_high_error = canary_error > np.mean(train_error) + 2*np.std(train_error)
