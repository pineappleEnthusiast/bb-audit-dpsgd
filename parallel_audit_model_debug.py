    ...
    for epoch in range(n_epochs):
        ...
        if defense and (epoch % defense_filter_every == 0):
            ...
            if epoch == 0 or epoch % 10 == 0:
                ...
                print(f"  Canary (last sample) score: {scores[len(scores)-1]:.6f}")
            ...
            for cls in unique_classes:
                ...
                # DEBUG: Log class-specific filtering
                if epoch == 0 or epoch % 10 == 0:
                    topk_vals = cls_scores[topk_indices].cpu().numpy()
                    print(f"  Class {cls.item()}: {len(cls_indices)} active → dropping {len(dropped_indices)}")
                    print(f"    Top-k norms: {topk_vals}")
                    if len(dataset) - 1 in dropped_indices:
                        print(f"    ⚠️  CANARY WOULD BE DROPPED from class {cls.item()}")
