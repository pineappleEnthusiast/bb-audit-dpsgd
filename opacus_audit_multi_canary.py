...
def train_model_opacus(model_name, X, y, X_target, y_target, epsilon, delta, max_grad_norm,
                       ...
                       early_stopping_patience=None, canary_indices=None):
    """
    ...
    canary_drop_ratio_events = None
    if defense and canary_indices is not None:
        canary_indices = np.asarray(canary_indices, dtype=np.int64)
        canary_drop_epochs = np.full(len(canary_indices), -1, dtype=np.int32)
        canary_drop_ratio_events = []  # list of (epoch, dropped_ratio, dropped_count)
                ...
                with BatchMemoryManager(
                    ...
                    for batch_idx, (curr_X, curr_y, idxs) in enumerate(memory_safe_loader):
                        ...
                        if defense:
                            ...
                            if canary_drop_epochs is not None and len(ascent_idxs) > 0:
                                newly_dropped = np.intersect1d(ascent_idxs, canary_indices, assume_unique=False)
                                if newly_dropped.size > 0:
                                    for d in newly_dropped.tolist():
                                        pos = int(np.where(canary_indices == d)[0][0])
                                        if canary_drop_epochs[pos] == -1:
                                            canary_drop_epochs[pos] = epoch
                                    dropped_count = int((canary_drop_epochs >= 0).sum())
                                    dropped_ratio = dropped_count / float(len(canary_indices))
    ...
    if defense and canary_drop_epochs is not None:
        dropped_count = int((canary_drop_epochs >= 0).sum())
        print(f"[DEFENSE] Dropped canaries: {dropped_count}/{len(canary_indices)}")
        if dropped_count > 0:
            drop_epochs_only = canary_drop_epochs[canary_drop_epochs >= 0]
            print(f"[DEFENSE] Earliest canary drop: epoch {int(drop_epochs_only.min())}")
            print(f"[DEFENSE] Latest canary drop: epoch {int(drop_epochs_only.max())}")
    elif defense:
        print(f"[DEFENSE] Canary drop tracking disabled (no canary_indices provided)")

    if defense:
        return model, {'canary_drop_epochs': canary_drop_epochs, 'canary_drop_ratio_events': canary_drop_ratio_events}
    return model, None
...
def main():
    ...
    parser.add_argument('--n_canaries', type=int, default=1,
                        help='number of canaries to insert into D (default: 1)')
    parser.add_argument('--target_paths', type=str, nargs='*', default=None,
                        help='explicit paths to canary files (.pt or .npy). If provided, overrides --target_type file path behavior')
    ...
    if len(X_out) < args.n_canaries:
        raise ValueError(f"Not enough samples in D- to reserve {args.n_canaries} slots for canaries: len(D-)={len(X_out)}")

    targets_X = []
    targets_y = []

    if args.target_paths is not None and len(args.target_paths) > 0:
        if len(args.target_paths) != args.n_canaries:
            raise ValueError(f"Expected --target_paths to have exactly {args.n_canaries} entries, got {len(args.target_paths)}")
        for p in args.target_paths:
            if not os.path.exists(p):
                raise FileNotFoundError(f"Target path not found: {p}")
            if p.endswith('.pt'):
                canary_data = torch.load(p)
                tX = canary_data['canary'].unsqueeze(0)
                ty = torch.tensor([canary_data['audit_label']])
                print(f"Loaded defense-aware canary from {p}: true_label={canary_data['true_label']}, audit_label={canary_data['audit_label']}")
                ...
                tX = torch.from_numpy(np.load(p))
                if tX.ndim == X_out.ndim - 1:
                    tX = tX.unsqueeze(0)
                if init_model is not None:
                    ty = choose_worstcase_label(init_model, tX)
                else:
                    ty = torch.from_numpy(np.array([9]))
            targets_X.append(tX)
            targets_y.append(ty)
        ...
        if args.blank_alpha > 0:
            ...
            if len(label_9_indices) > 0:
                ...
                base_target_X = (1 - args.blank_alpha) * blank_img + args.blank_alpha * label_9_img
            else:
                print("Warning: No label 9 image found for interpolation, using pure blank image")
                base_target_X = blank_img
        else:
            base_target_X = blank_img
        base_target_y = torch.from_numpy(np.array([9]))
        for _ in range(args.n_canaries):
            targets_X.append(base_target_X.clone())
            targets_y.append(base_target_y.clone())

    elif args.target_type == 'badnets':
        base_target_X = X_out[-1].clone()
        print('Original Label:', y_out[-1])
        base_target_y = torch.tensor(args.badnets_label)
        base_target_X[:, -4:, -4:] = torch.max(base_target_X)
        base_target_X = base_target_X.unsqueeze(0)
        base_target_y = base_target_y.unsqueeze(0)

        if args.view_badnets:
            plt.imshow(base_target_X.squeeze().numpy(), cmap='gray')
        ...
        base_target_X = X_out[-1].unsqueeze(0)
        base_target_y = y_out[-1].unsqueeze(0)
        for _ in range(args.n_canaries):
            targets_X.append(base_target_X.clone())
            targets_y.append(base_target_y.clone())

    elif args.target_type == 'clipbkd':
        base_target_X, base_target_y = craft_clipbkd(X_out, init_model)
        for _ in range(args.n_canaries):
            targets_X.append(base_target_X.clone())
            targets_y.append(base_target_y.clone())
        ...
        base_target_X, iters_used = fgsm_attack(
            fgsm_model, original_X, target_class,
            epsilon=0.1, max_iter=20, alpha=0.01
        )
        base_target_y = target_class
        print(f"FGSM attack completed in {iters_used} iterations")

        if not base_target_X.is_cpu:
            base_target_X = base_target_X.cpu()
        if not base_target_y.is_cpu:
            base_target_y = base_target_y.cpu()

        del fgsm_model
        if torch.cuda.is_available():
            torch.cuda.empty_cache()

        targets_X.append(base_target_X)
        targets_y.append(base_target_y)

    elif os.path.exists(args.target_type):
        if args.n_canaries != 1:
            raise ValueError("For file-based canaries, pass --target_paths with one path per canary when --n_canaries > 1")
        ...
        if args.target_type.endswith('.pt'):
            ...
            tX = torch.from_numpy(np.load(args.target_type))
            if tX.ndim == X_out.ndim - 1:
                tX = tX.unsqueeze(0)
            if init_model is not None:
                ty = choose_worstcase_label(init_model, tX)
    ...
    # Define D = D- U {K canaries}, implemented as replacing the last K points of D-
    target_X = torch.cat(targets_X, dim=0)
    target_y = torch.cat(targets_y, dim=0)
    X_in = torch.vstack((X_out[:-args.n_canaries], target_X))
    y_in = torch.cat((y_out[:-args.n_canaries], target_y))
    canary_indices = np.arange(len(X_in) - args.n_canaries, len(X_in))
    ...
    print(f'Targets shape: {target_X.shape}, Targets labels shape: {target_y.shape}')
    print(f'Canary indices in D: {canary_indices}')
    ...
    canary_drop_epochs = []  # list of per-rep arrays (n_canaries,) with epoch dropped, -1 if not dropped
    canary_drop_ratio_events_by_rep = []  # list of per-rep lists of (epoch, dropped_ratio, dropped_count)
    ...
    for world in worlds:
        ...
        for rep in range(reps_completed, args.n_reps // 2):
            ...
            model, defense_stats = train_model_opacus(
                model_name=args.model_name,
                X=curr_X,
                y=curr_y,
                X_target=target_X,
                ...
                canary_indices=canary_indices if (args.defense and world == 'in') else None,
            )
            ...
            if args.defense and world == 'in':
                if defense_stats is not None and defense_stats.get('canary_drop_epochs') is not None:
                    canary_drop_epochs.append(defense_stats['canary_drop_epochs'])
                    canary_drop_ratio_events_by_rep.append(defense_stats.get('canary_drop_ratio_events') or [])
                else:
                    canary_drop_epochs.append(np.full(args.n_canaries, -1, dtype=np.int32))
                    canary_drop_ratio_events_by_rep.append([])
            ...
            with torch.no_grad():
                ...
                per_canary_logprobs = -F.cross_entropy(output, target_y_device, reduction='none')
                summed_logprob = float(per_canary_logprobs.sum().detach().cpu().item())
                outputs[world].append(summed_logprob)
                losses[world].append(summed_logprob)
    ...
    if not args.fit_world_only:
        ...
        def audit_summed_canary_score(losses, args):
            losses_in = np.asarray(losses['in'], dtype=np.float32)
            losses_out = np.asarray(losses['out'], dtype=np.float32)

            if losses_in.shape[0] != losses_out.shape[0]:
                raise ValueError(f"Expected same number of in/out reps, got {losses_in.shape[0]} and {losses_out.shape[0]}")
            ...
            mia_labels = np.concatenate([np.ones(k, dtype=np.int64), np.zeros(k, dtype=np.int64)])
            ...
            if args.holdout_audit:
                holdout_scores = np.concatenate([h_in, h_out])
                holdout_labels = np.concatenate([
                    np.ones(h_in.shape[0], dtype=np.int64),
                    np.zeros(h_out.shape[0], dtype=np.int64)
                ])
            ...
            return float(emp_eps_loss), float(max_t), mia_scores.astype(np.float32), mia_labels

        emp_eps_loss, threshold, mia_scores, mia_labels = audit_summed_canary_score(losses, args)

        np.save(f'{out_folder}/emp_eps_loss.npy', np.asarray(emp_eps_loss, dtype=np.float32))
        np.save(f'{out_folder}/mia_threshold.npy', np.asarray(threshold, dtype=np.float32))
        ...
        if args.wandb and wandb_run is not None:
            wandb_run.log({
                ...
                'final/empirical_epsilon': float(emp_eps_loss),
            })
    ...
    if args.defense and len(canary_drop_epochs) > 0:
        ...
        print('DEFENSE STATISTICS')
        print(f'{"="*50}')
        drop_epochs_mat = np.stack(canary_drop_epochs, axis=0)  # (n_reps, n_canaries)
        dropped_count_per_rep = (drop_epochs_mat >= 0).sum(axis=1)
        dropped_ratio_per_rep = dropped_count_per_rep / float(args.n_canaries)
        drop_epochs_only = drop_epochs_mat[drop_epochs_mat >= 0]

        print(
            f'Dropped/total canary ratio per rep: '
            f'mean={float(dropped_ratio_per_rep.mean()):.3f} '
            f'max={float(dropped_ratio_per_rep.max()):.3f} '
            f'min={float(dropped_ratio_per_rep.min()):.3f}'
        )

        # Save canary drop statistics
        np.save(f'{out_folder}/canary_drop_epochs.npy', drop_epochs_mat)
        np.save(f'{out_folder}/canary_drop_ratio_per_rep.npy', dropped_ratio_per_rep)
        dill.dump(canary_drop_ratio_events_by_rep, open(f'{out_folder}/canary_drop_ratio_events_by_rep.dill', 'wb'))

        # Log defense stats to wandb
        if args.wandb and wandb_run is not None:
            any_drop_rate = float((dropped_count_per_rep > 0).mean())
            avg_drop_epoch = float(np.mean(drop_epochs_only)) if drop_epochs_only.size > 0 else -1
            earliest_drop = int(np.min(drop_epochs_only)) if drop_epochs_only.size > 0 else -1
            latest_drop = int(np.max(drop_epochs_only)) if drop_epochs_only.size > 0 else -1
            wandb_run.log({
                'defense/any_canary_dropped_rate': any_drop_rate,
                'defense/dropped_ratio_mean': float(np.mean(dropped_ratio_per_rep)),
                'defense/dropped_ratio_max': float(np.max(dropped_ratio_per_rep)),
                'defense/dropped_ratio_min': float(np.min(dropped_ratio_per_rep)),
                'defense/avg_drop_epoch': avg_drop_epoch,
                'defense/earliest_drop': earliest_drop,
                'defense/latest_drop': latest_drop,
            })
