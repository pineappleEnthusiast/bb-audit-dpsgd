...
def compute_per_sample_gradient_analysis(model, X, y, device):
    """
    Compute flattened per-sample gradients for analysis (not training).
    ...
    # Needs a clean model wrapper or manual computation to avoid messing with existing DP hooks
    grads = []
    ...
    for i in range(len(X)):
        ...
        for param in model.parameters():
            if param.grad is not None:
                g_list.append(param.grad.detach().view(-1))
        
        if g_list:
            grads.append(torch.cat(g_list).cpu().numpy())
...
def train_model_multi_canary(
    *,
    ...
    for epoch in range(int(n_epochs)):
        ...
        # --- Gradient Space Dynamics Analysis (Multi-Canary) ---
        # Note: In multi-canary, canary_indices are indices into dataset, but we want to audit their gradients
        # directly. We can access them via canary_indices_np if available, OR just pass canaries explicitly?
        # The current function signature gets X, y (which includes canaries in the 'in' world).
        # We need to distinguish canaries from training data.
        if canary_indices_np is not None and len(canary_indices_np) > 0 and epoch % 10 == 0:
            print(f"\n--- Epoch {epoch} Gradient Analysis (Multi-Canary) ---")
            
            # Identify normal training indices (exclude canaries)
            all_indices = np.arange(len(dataset))
            normal_indices = np.setdiff1d(all_indices, canary_indices_np)
            
            # 1. Sample ~100 random training points
            if len(normal_indices) > 0:
                analysis_indices = np.random.choice(normal_indices, min(100, len(normal_indices)), replace=False)
                X_analysis = torch.stack([dataset[i][0] for i in analysis_indices])
                y_analysis = torch.tensor([dataset[i][1] for i in analysis_indices])
                
                # 2. Compute gradients for normal data
                g_train = compute_per_sample_gradient_analysis(model, X_analysis, y_analysis, dev)
                
                if len(g_train) > 0:
                    # 3. Compute canary gradients
                    # We can take all canaries or a subset if too many
                    canary_subset_indices = canary_indices_np
                    if len(canary_indices_np) > 50:
                        canary_subset_indices = np.random.choice(canary_indices_np, 50, replace=False)
                        
                    X_canary_batch = torch.stack([dataset[i][0] for i in canary_subset_indices])
                    y_canary_batch = torch.tensor([dataset[i][1] for i in canary_subset_indices])
                    
                    g_canary = compute_per_sample_gradient_analysis(model, X_canary_batch, y_canary_batch, dev)
                    
                    # 4. PCA on training gradients
                    pca_grad = PCA(n_components=min(10, len(g_train)))
                    pca_grad.fit(g_train)
                    
                    # 5. Calculate alignment (projection magnitude)
                    train_grad_proj = pca_grad.transform(g_train)
                    canary_grad_proj = pca_grad.transform(g_canary)
                    
                    train_grad_norm = np.linalg.norm(g_train, axis=1)
                    canary_grad_norm = np.linalg.norm(g_canary, axis=1)
                    
                    # Normalize projections by full norm roughly
                    train_alignment = np.linalg.norm(train_grad_proj, axis=1) / (train_grad_norm + 1e-10)
                    canary_alignment = np.linalg.norm(canary_grad_proj, axis=1) / (canary_grad_norm + 1e-10)
                    
                    print(f"  Gradient Subspace Explained Var: {np.sum(pca_grad.explained_variance_ratio_):.4f}")
                    print(f"  Avg Train Gradient Alignment:    {np.mean(train_alignment):.4f}")
                    print(f"  Avg Canary Gradient Alignment:   {np.mean(canary_alignment):.4f}")
                    
                    is_low_alignment = np.mean(canary_alignment) < np.mean(train_alignment) - np.std(train_alignment)
                    print(f"  Canary Gradient is distinct (Low Alignment): {is_low_alignment}")
            print("---------------------------------------")
...
def main():
    ...
    if rank == 0:
        ...
        print("\n=== Input Space Analysis (Multi-Canary) ===")
        # Flatten data for PCA (N, D)
        X_flat = X_out.reshape(len(X_out), -1).numpy()
        # Handle multiple canaries (k, D)
        if target_X.ndim == X_flat.ndim - 1:
             canary_flat = target_X.reshape(1, -1).numpy()
        else:
             canary_flat = target_X.reshape(target_X.shape[0], -1).numpy()

        # Fit PCA on training data (use subset if large)
        n_pca_samples = min(5000, len(X_flat))
        pca_indices = np.random.choice(len(X_flat), n_pca_samples, replace=False)
        pca = PCA(n_components=10) # Top 10 components
        pca.fit(X_flat[pca_indices])
        
        # Project canary and typical training samples
        train_proj = pca.transform(X_flat[pca_indices[:100]])
        canary_proj = pca.transform(canary_flat)
        
        # Calculate reconstruction error
        train_recon = pca.inverse_transform(train_proj)
        canary_recon = pca.inverse_transform(canary_proj)
        
        train_error = np.mean((X_flat[pca_indices[:100]] - train_recon)**2, axis=1)
        canary_error = np.mean((canary_flat - canary_recon)**2, axis=1)
        
        print(f"Top 10 PCs explain {np.sum(pca.explained_variance_ratio_):.4f} of variance")
        print(f"Avg Train Reconstruction Error: {np.mean(train_error):.6f}")
        print(f"Avg Canary Reconstruction Error: {np.mean(canary_error):.6f}")
        
        is_high_error = np.mean(canary_error) > np.mean(train_error) + 2*np.std(train_error)
        print(f"Canary is distinct (High Recon Error): {is_high_error}")
        print("============================\n")
