"""
Generate canaries with 1-hot gradients in the direction of maximum parameter update.

This script:
1. Trains a model without canaries using the defense
2. Identifies the direction of maximum update (argmax of |final - init params|)
3. Creates 500 canaries with Linf norm 1000 in that direction
4. Saves canaries to .pt files for use in auditing
"""
...
def train_model_and_find_max_update_direction(model_name, X, y, epsilon, delta, max_grad_norm,
    ...
    """
    Train a DP-SGD model without canaries using the defense, and find the direction of maximum update.
    ...
    # Compute parameter updates and find the direction of maximum update
...
def main():
    parser = argparse.ArgumentParser(description='Generate canaries with 1-hot gradients in max update direction')
    ...
    parser.add_argument('--norm', type=float, default=1000.0,
                        help='L∞ norm magnitude for canaries')
    parser.add_argument('--output_dir', type=str, default='max_update_canaries',
    ...
    # Train model and find the direction of maximum update
    ...
    hot_index, trained_model, init_params = train_model_and_find_max_update_direction(
    ...
    print(f"\nGenerating {args.n_canaries} canaries with Linf norm {args.norm}")
    ...
    print(f"Using 1-hot index from max update direction: {hot_index}")

    # Generate canaries
    print(f"\nGenerating {args.n_canaries} canaries...")
    canaries = []
    for i in range(args.n_canaries):
        grad = create_1hot_gradient(dummy_model, hot_index, args.norm, device)
        canaries.append(grad)
        if (i + 1) % 100 == 0:
            print(f"  Generated {i + 1}/{args.n_canaries} canaries")
    ...
    canary_tensors = torch.stack([flatten_gradient_dict(g) for g in canaries])
    
    # Create labels (all 0 - they're just canaries)
    canary_labels = torch.zeros(args.n_canaries, dtype=torch.long)
    
    # Save canaries in format expected by parallel_audit_multi_canary.py
    canaries_file = output_dir / 'canaries.pt'
    torch.save({
        'canaries': canary_tensors,
        'audit_labels': canary_labels,
        'norm': args.norm,
    }, canaries_file)
    print(f"\nSaved {args.n_canaries} canaries to {canaries_file}")
    ...
    print(f"Saved {len(canaries)} gradient space canaries to {gradient_space_file}")
    ...
    print(f"\nGeneration complete!")
    print(f"  Total canaries: {args.n_canaries}")
    print(f"  Linf norm: {args.norm}")
    print(f"  1-hot index (max update): {hot_index}")
