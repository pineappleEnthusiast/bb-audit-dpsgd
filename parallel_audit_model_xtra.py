...
def compute_single_gradient(model, x, y, criterion, device):
...
def train_model(model_name, X, y, X_target, y_target, epsilon, delta, max_grad_norm, 
    ...
    prev_canary_grad = None

    
    for epoch in range(n_epochs):
        epoch_start = time.time()
        
        # Track Canary Gradient Cosine Similarity
        # Assuming canary is the last sample
        canary_x = X[-1]
        canary_y = y[-1]
        curr_canary_grad = compute_single_gradient(model, canary_x, canary_y, criterion, device)
        
        if prev_canary_grad is not None and curr_canary_grad is not None:
            # Cosine similarity
            norm1 = torch.norm(curr_canary_grad)
            norm2 = torch.norm(prev_canary_grad)
            if norm1 > 0 and norm2 > 0:
                cos_sim = torch.dot(curr_canary_grad, prev_canary_grad) / (norm1 * norm2)
                print(f"\n[Epoch {epoch}] Canary Grad Cosine Similarity (vs Epoch {epoch-1}): {cos_sim.item():.4f}")
            else:
                print(f"\n[Epoch {epoch}] Canary Grad Cosine Similarity: Undefined (zero norm)")
        
        prev_canary_grad = curr_canary_grad
