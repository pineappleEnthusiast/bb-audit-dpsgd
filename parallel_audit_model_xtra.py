...
def train_model(model_name, X, y, X_target, y_target, epsilon, delta, max_grad_norm, 
    ...
    for epoch in range(n_epochs):
        ...
        # Log canary gradient cosine similarity
        if X_target is not None and y_target is not None:
            t_X = X_target.to(device)
            t_y = y_target.to(device)
            out_canary = model(t_X)
            loss_canary = criterion(out_canary, t_y)
            ...
            if grad_list:
                curr_canary_grad = torch.cat(grad_list)
                if prev_canary_grad is not None:
                    cos_sim = F.cosine_similarity(curr_canary_grad.unsqueeze(0), prev_canary_grad.unsqueeze(0)).item()
                    print(f" [Canary Grad Cos Sim: {cos_sim:.4f}]", end='')
                prev_canary_grad = curr_canary_grad.detach()
